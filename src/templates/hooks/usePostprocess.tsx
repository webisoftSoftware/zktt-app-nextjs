...
type Uniforms = {
  diffuse: { value: THREE.Texture | null };
  time: { value: number };
};
...
const usePostProcess = () => {
  ...
  const [screenCamera, screenScene, screen, renderTarget] = useMemo(() => {
    ...
    const screen = new THREE.Mesh(getFullscreenTriangle(), new THREE.RawShaderMaterial(shaderMaterial));
    screen.frustumCulled = false;
    (screen.material.uniforms as Uniforms).diffuse.value = renderTarget.texture;
  ...
  useFrame(({ scene, camera, gl }, delta) => {
    ...
    if (screen) (screen.material.uniforms as Uniforms).time.value += delta;
